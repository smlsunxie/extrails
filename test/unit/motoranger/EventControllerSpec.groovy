...
@Mock([Event, EventDetail, User, Product, UserService])
class EventControllerSpec extends Specification {
    ...
    def populateValidParams(params) {
        ...
        params["name"] = 'event1'
        params["product.id"] = Product.findByName("product").id
        params["date"] = new Date()
        params["user.id"] = User.findByUsername("user").id
    }
    ...
    void "Test the save action correctly persists an instance"() {
        ...
        then:"A redirect is issued to the show action"
            response.redirectedUrl == '/event/pickPartAddDetail/'+event.id
